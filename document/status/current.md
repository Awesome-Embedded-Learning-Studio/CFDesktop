...
3. **MS4 应用启动器**（应用网格 + QProcess 启动 + 双态框架）— 🚧 进行中（启动闭环 + 双态框架落地：`AppLaunchService` 真启动；`LaunchKind`/`IBuiltinPanel`/`BuiltinPanelRegistry` 把 `builtin:` hack 正式化、消灭 if 链,calculator 双态,`HardwareTier::prefer_inprocess_apps` 按 Low/High 裁决 manifest `launch_kind:"auto"`;开始菜单弹窗网格待做。详见 [milestone_04 §六](../todo/desktop/milestone_04_app_launcher.md)）
