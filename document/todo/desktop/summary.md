---
title: CFDesktop 桌面本体规划文档
description: CFDesktop 桌面本体规划文档 的详细文档
---
...
CFDesktop 是一个基于 **Qt 6.8.3+ / C++23** 开发的**嵌入式桌面 UI 框架**，遵循 Material Design 3 规范。当前版本（0.13.1）本质上是一个**UI 组件库 + 底层架构**，尚不构成完整的桌面环境。
...
| ThemeEngine 主题管理（QuarkWidgets MD3 引擎） | 100% | ✅ UI 框架主题引擎；桌面 ThemeStyleManager + iOS/Win11 主题包 ⬜0%（Phase G 待做） |
| AnimationManager 动画引擎 | 100% | ✅ 桌面过渡动效基础 |
| DPI 管理 | 部分 | ⚠️ 仅组件层 devicePixelRatioF 隐式处理；独立 DPIManager 注入（Phase B B3）待做 |
...
| 显示服务器抽象 (IDisplayServerBackend) | 100% | ✅ 仅三模式接口 + outputs() 只读枚举；RenderBackend 具体后端与 Factory 实现 0%（Phase B 待做，6ULL 必需的 EGLFS/LinuxFB 待做） |
...
| HWTier 硬件分级系统 | ✅ 已完成 | 检测/评分/Assessor/Policy 接口 + DefaultPolicy 已落地（`base/system/hardware_tier/`）；仅 CapabilityPolicy 策略引擎（档位→降级接 Shell）延后 |
| CrashHandler 崩溃处理 | 0% | 🟡 稳定性保障 |
| IPC 进程间通信 | 0% | 🔴 多进程架构硬阻塞：CrashHandler Phase2 / NotificationService / MediaService / 设置 App 跨进程 / Widget 进程隔离均依赖 |
...
**A2 — ConfigStore 配置中心**
- 四层存储模型：System → User → App → Temp（与 1.2 节 / current.md 对齐）
- 变更监听机制（Observer 模式）
...
**A5 — IPC 基础层** ⬜ 0% 待实现（非"已设计"——代码核实 `IPCMessage/IPCClient/IPCServer/ServiceLocator` 零命中）
> 🔴 硬阻塞项：CrashHandler Phase2 / NotificationService / MediaService / 系统设置 App 跨进程 / Widget 进程隔离均依赖本层，须作为基础设施首批落地。绝不引入 D-Bus（嵌入式不可用）。single-instance 锁（QLockFile）成本最低，优先做。
- QLocalSocket 封装（进程间消息信道）
- 消息序列化协议定义
- 服务注册/发现机制（ServiceLocator）
- single-instance 单例锁（QLockFile / lockfile）
- 基础测试覆盖
