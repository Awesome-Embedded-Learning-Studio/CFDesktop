...
- [ ] `HWTier` 枚举定义及查询接口
- [ ] ~~设备配置文件 `/etc/CFDesktop/device.conf`~~ ⚠️ **过时**: 实际实现使用 `setDeviceConfigOverride()` API + CFDesktop 自管理目录，不读取系统级路径
- [ ] Mock 数据集用于单元测试
 ...
 * @brief 设备配置文件读取器
 *
 * ⚠️ **过时**: 实际实现使用 `setDeviceConfigOverride()` API，配置在 CFDesktop 自管理目录内。
 * 以下 `/etc/CFDesktop/` 路径仅为早期设计参考。
 *
...
public:
     ...
     * @brief 加载配置文件
     */
    static Config load(const QString& path = "/etc/CFDesktop/device.conf"); // ⚠️ 过时：实际使用应用内目录

    /**
     * @brief 保存配置文件
     */
    static bool save(const Config& config,
                     const QString& path = "/etc/CFDesktop/device.conf"); // ⚠️ 过时
...
> ⚠️ **过时说明**: 以下 `/etc/CFDesktop/` 路径为早期设计。实际实现中，CFDesktop 不读取系统级路径，设备配置通过 `setDeviceConfigOverride()` API 或 ConfigStore 应用内目录管理。
