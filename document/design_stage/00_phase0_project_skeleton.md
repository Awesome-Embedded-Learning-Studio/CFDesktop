...
│   └── workflows/
│       ├── build.yml          # 主构建流程
│       ├── build-arm.yml      # ARM 交叉编译
│       └── deploy.yml         # 自动部署流程
│
├── cmake/
│   ├── modules/
│   │   ├── FindQt6.cmake       # Qt6 查找模块
│   │   ├── CompilerWarnings.cmake  # 编译警告配置
│   │   └── PlatformDetect.cmake    # 平台检测
...
| 目录 | 职责 | 产出物 |
|------|------|--------|
| `src/base/` | 基础设施库，无 UI 依赖 | `libCFDesktopBase.so/a` |
| `src/sdk/` | 对外 API 导出层 | `libCFDesktopSDK.so/a` |
| `src/shell/` | 桌面 Shell 主程序 | `cfdesktop-shell` 可执行文件 |
| `src/simulator/` | PC 端模拟器 | `cfdesktop-sim` 可执行文件 |
| `tests/` | 所有测试代码 | 测试可执行文件 |
| `apps/` | 示例和模板应用 | `.desktop` 文件 + 二进制 |
