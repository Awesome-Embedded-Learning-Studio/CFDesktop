/**
 * @file    cpu_card.cpp
 * @brief   Implementation of the CpuCard.
 *
...
CpuCard::CpuCard(QWidget* parent) : QWidget(parent) {
...
void CpuCard::refresh() {
    auto result = cf::getCPUProfileInfo();
    if (result.has_value()) {
        const auto& profile = result.value();
        pct_ = static_cast<int>(profile.cpu_usage_percentage);
        if (pct_ < 0) {
            pct_ = 0;
        }
        if (pct_ > 100) {
        ...
        detail_text_ = QStringLiteral("%1 cores · %2 MHz")
                           .arg(profile.logical_cnt)
                           .arg(profile.current_frequecy);
    }
    update();
}

void CpuCard::applyTheme() {
    try {
...
void CpuCard::paintEvent(QPaintEvent* /*event*/) {
    QPainter p(this);
